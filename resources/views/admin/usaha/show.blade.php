...
@php
    ...
    $statusBadgeClass = '';
    switch($usaha->status) {
        case 'menunggu': $statusBadgeClass = 'bg-amber-100 text-amber-800 border-amber-200'; break;
        case 'disetujui': $statusBadgeClass = 'bg-emerald-100 text-emerald-800 border-emerald-200'; break;
        case 'ditolak': $statusBadgeClass = 'bg-red-100 text-red-800 border-red-200'; break;
        default: $statusBadgeClass = 'bg-slate-100 text-slate-800 border-slate-200';
    }
@endphp
    ...
    <div class="lg:col-span-8 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border {{ $statusBadgeClass }}">
                    {{ $usaha->status_label }}
                </span>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-slate-700 mt-1">{{ $usaha->user->name }} <span class="text-slate-400 text-sm">({{ $usaha->user->email }})</span></p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded text-sm font-medium bg-slate-100 text-slate-800 border border-slate-200">
                    ...
                    <div>
                        ...
                        <p class="text-slate-700 font-mono text-sm mt-1">{{ $usaha->latitude }}, {{ $usaha->longitude }}</p>
                    </div>
                    
                    <!-- Tanggal -->
                    <div>
                        <label class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Tanggal Pengajuan</label>
                ...
                @if($usaha->dokumens->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        @foreach($usaha->dokumens as $dokumen)
                            <div class="border border-slate-200 rounded-xl p-4 text-center hover:border-indigo-300 hover:shadow-md transition-all group">
                                <div class="w-12 h-12 mx-auto bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center mb-3 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                                    <i class="bi bi-file-earmark-text text-2xl"></i>
                                </div>
                                <p class="text-sm font-medium text-slate-800 truncate mb-2" title="{{ $dokumen->jenis_label }}">{{ $dokumen->jenis_label }}</p>
                                <a href="{{ asset('storage/' . $dokumen->file_path) }}" target="_blank" class="inline-flex items-center text-xs font-medium text-indigo-600 hover:text-indigo-800">
    ...
    <div class="lg:col-span-4 space-y-6">
        ...
        @if($usaha->foto_usaha)
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="p-0">
                    <img src="{{ asset('storage/' . $usaha->foto_usaha) }}" class="w-full h-auto object-cover" alt="Foto Usaha">
                </div>
            </div>
        @endif
        ...
        @if($usaha->status === 'menunggu_admin')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="p-6">
                    <p class="text-sm text-slate-500 mb-4">Pengajuan akan diverifikasi dan diteruskan ke Camat untuk persetujuan akhir.</p>
                    <form action="{{ route('admin.usaha.approve', $usaha) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2.5 rounded-lg transition-colors shadow-sm shadow-emerald-200">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="p-6">
                    <form action="{{ route('admin.usaha.reject', $usaha) }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label for="catatan_penolakan" class="block text-sm font-medium text-slate-700 mb-2">Catatan Penolakan <span class="text-red-500">*</span></label>
                        ...
                        <button type="submit" class="w-full flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-medium py-2.5 rounded-lg transition-colors shadow-sm shadow-red-200">
        ...
        @if($usaha->surat_izin)
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="p-6 text-center">
                    ...
                    <a href="{{ asset('storage/' . $usaha->surat_izin) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg text-sm font-medium transition-colors">
        ...
        <!-- Tombol Kembali -->
        <a href="{{ route('admin.usaha.index') }}" class="block w-full text-center border border-slate-300 text-slate-600 hover:bg-slate-50 hover:text-slate-800 font-medium py-3 rounded-xl transition-colors">
