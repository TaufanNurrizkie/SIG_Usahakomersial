...
<head>
    ...
    <style>
        ...
        #map {
            height: 400px;
            z-index: 1;
            /* Mencegah peta menutupi dropdown/modal */
        }
...
<body class="bg-slate-50 text-slate-800 antialiased">
    ...
    <div id="sidebarOverlay" onclick="toggleSidebar()"
        class="fixed inset-0 bg-black/50 z-40 hidden lg:hidden glass transition-opacity"></div>

    <!-- Sidebar -->
    <aside id="sidebar"
        class="fixed top-0 left-0 h-screen w-64 bg-sidebar text-white z-50 transition-transform duration-300 -translate-x-full lg:translate-x-0 shadow-xl flex flex-col">
        <!-- Brand -->
    ...
    <main class="lg:ml-64 min-h-screen flex flex-col transition-all duration-300">
        ...
        <!-- Top Navbar -->
        <nav
            class="bg-white border-b border-slate-200 px-4 py-3 sticky top-0 z-30 shadow-sm flex justify-between items-center">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="relative">
                    <button onclick="toggleNotif()"
                        class="relative p-2 rounded-lg hover:bg-slate-100 text-slate-600 hover:text-slate-800 transition-colors">

                        <i class="bi bi-bell text-xl"></i>

                        @if ($notifications->count())
                            <span
                                class="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-xs flex items-center justify-center">
                                {{ $notifications->count() }}
                            </span>
                        @endif
                    </button>
                    ...
                    <div id="notifDropdown"
                        class="hidden absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg border border-slate-100 z-50 overflow-hidden">
                            ...
                            @forelse ($notifications as $notif)
                                <a href="{{ route('notifications.read', $notif->id) }}"
                                    ...
                                    <div class="font-semibold text-slate-800">
                                        {{ $notif->data['message'] ?? 'Ada pembaruan pengajuan usaha' }}
                                    </div>

                                    <div class="text-sm text-slate-600">
                                        {{ $notif->data['nama_usaha'] }}
                                    </div>
                ...
                <div class="relative">
                    <button onclick="toggleDropdown()"
                        class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-slate-100 text-slate-700 transition-colors">
                    ...
                    <div id="userDropdown"
                        class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-100 py-1 z-50">
        ...
        <div class="p-4 md:p-6 lg:p-8 flex-1">
            ...
            <!-- Flash Messages -->
            @if (session('success'))
                <div class="flex items-center gap-3 p-4 mb-6 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl shadow-sm"
                    role="alert">
            ...
            @if (session('error'))
                <div class="flex items-center gap-3 p-4 mb-6 bg-red-50 text-red-700 border border-red-200 rounded-xl shadow-sm"
                    role="alert">
    ...
    <script>
        ...
        window.addEventListener('click', function(e) {
            ...
            const bell = e.target.closest('.bi-bell');

            if (notif && !notif.contains(e.target) && !bell) {
