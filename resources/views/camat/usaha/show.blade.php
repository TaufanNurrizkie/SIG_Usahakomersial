        ...
        <div class="lg:col-span-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                <div class="px-4 py-3 bg-white border-b border-gray-200 flex justify-between items-center">
                    ...
                    ?>
                    <span
                        class="{{ $bgClass }} text-xs font-medium px-2.5 py-0.5 rounded">{{ $usaha->status_label }}</span>
                </div>
                <div class="p-4 bg-white">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="mb-0"><span
                                    class="bg-gray-100 text-gray-800 text-xs font-medium px-2.5 py-0.5 rounded">{{ $usaha->kategori->nama_kategori }}</span>
                            </p>
                        </div>
                        <div>
        ...
        <div class="lg:col-span-4">
            ...
            @if ($usaha->foto_usaha)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                    ...
                    <div class="p-0 bg-white">
                        <img src="{{ asset('storage/' . $usaha->foto_usaha) }}" class="w-full h-auto rounded-b-lg"
                            alt="Foto Usaha">
                    </div>
                </div>
            @endif
            ...
            @if ($usaha->status === 'menunggu_camat')
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                    ...
                    <div class="p-4 bg-white">
                        <form action="{{ route('camat.usaha.approve', $usaha) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label class="block font-medium text-sm text-gray-700 mb-1">Upload Surat Izin (PDF) <span
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                    ...
                    <div class="p-4 bg-white">
                        <form action="{{ route('camat.usaha.reject', $usaha) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label class="block font-medium text-sm text-gray-700 mb-1">Catatan Penolakan <span
                                        class="text-red-500">*</span></label>
                                <textarea name="catatan_penolakan"
                                    class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block w-full"
                                    rows="3" required placeholder="Alasan penolakan..."></textarea>
                            </div>
                            <button type="submit"
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-4">
                    ...
                    <div class="p-4 bg-white text-center">
                        ...
                        <a href="{{ asset('storage/' . $usaha->surat_izin) }}" target="_blank"
                            class="inline-flex items-center px-4 py-2 bg-white border border-red-600 rounded-md font-semibold text-xs text-red-600 uppercase tracking-widest hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
