...
@section('title', 'Persetujuan Usaha')

@section('content')
<!-- Header Halaman -->
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
    <h2 class="text-xl font-bold text-slate-800">Persetujuan Usaha</h2>
    <div class="text-sm text-slate-500">
        Halaman verifikasi persetujuan izin usaha
    </div>
</div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-6 overflow-hidden">
    <div class="p-5 border-b border-gray-100 bg-gray-50">
        <h3 class="font-semibold text-gray-800 text-sm flex items-center gap-2">
            <i class="bi bi-funnel-fill text-indigo-500"></i> Filter Data
        </h3>
    </div>
    <div class="p-5">
        <form action="{{ route('camat.usaha.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div class="md:col-span-3">
                ...
                <select name="status" class="w-full rounded-lg bg-white border-2 border-gray-200 text-gray-700 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all px-3 py-2 text-sm">
                    <option value="menunggu" {{ request('status', 'menunggu') == 'menunggu' ? 'selected' : '' }}>Menunggu Persetujuan</option>
            ...
            <div class="md:col-span-3 flex items-end gap-2">
                ...
                <a href="{{ route('camat.usaha.index') }}" class="px-4 py-2 border border-gray-300 text-gray-600 hover:bg-gray-50 rounded-lg text-sm font-medium transition-colors flex items-center justify-center">
                    Reset
                </a>
            </div>
        </form>
        ...
        <table class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-4 font-bold bg-gray-50">Tanggal</th>
            ...
            <tbody>
                @forelse($usahas as $usaha)
                    <tr class="bg-white border-b border-gray-100 hover:bg-indigo-50/30 transition-colors">
                        ...
                        <td class="px-6 py-4 font-mono text-xs">{{ $usaha->created_at->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('camat.usaha.show', $usaha) }}" class="inline-flex items-center px-4 py-2 border border-indigo-200 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-lg text-sm font-medium transition-colors">
                    ...
                    <tr class="bg-white border-b border-gray-100">
                        <td colspan="7" class="px-6 py-12 text-center text-gray-400">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <span class="font-medium">Tidak ada data usaha ditemukan</span>
                            </div>
                        </td>
                    </tr>
                @endforelse
